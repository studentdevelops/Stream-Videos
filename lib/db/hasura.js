...
export const InsertIntoStats = async ({ UserId, VideoId, Favorite, watched }, token) => {
    // ($UserId: String!, VideoId: String!, $Favorite: Integer!, $watched: Boolean!)
    const operationsDoc = `
  mutation InsertStats($UserId: String!, $VideoId: String!, $Favorite: Int!, $watched: Boolean!) {
    insert_stats(objects: {Favorited: $Favorite, UserId: $UserId, VideoId: $VideoId, watched: $watched}) {
      returning {
        Favorited
        UserId
        VideoId
...
`;
    const response = await QueryHasuraGQL(operationsDoc, "InsertStats", { VideoId, UserId, Favorite, watched }, token);
